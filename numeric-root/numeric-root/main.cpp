...
#define MAXITER 50
#define DBL_RADIX       2
#define TIMVALFORMAT(tvalv) (((double)(tvalv.tv_sec))+((double)(tvalv.tv_usec))/((double)1e6))
#define TIMSPEFORMAT(tvalv) (((double)(tvalv.tv_sec))+((double)(tvalv.tv_nsec))/((double)1e9))

#define TIMESPECFROMUS(us, tvspec)  {tvspec.tv_sec=us/1e6; \
                                    int rem_ns=(us%1000000)*1000; \
                                    tvspec.tv_nsec=rem_ns;}
...
static double upperbound( const int n, const double a[], const complex<double> z )
   ...
   e = ( 9.0 * e - 7.0 * ( fabs( t ) + fabs( r ) * u ) +
       fabs( z.real() ) * fabs( r ) * 2.0 ) * pow( DBL_RADIX, -DBL_MANT_DIG+1);
...
int newton_real( register int n, const double coeff[], complex<double> res[] )
   ...
   while( n > 2 )
      ...
      eps = 4 * n * n * f0 * pow( DBL_RADIX, -DBL_MANT_DIG * 2 );
...
iter2:
         ...
         if( r < sqrt( z.real() * z.real() + z.imag() * z.imag() ) * pow( DBL_RADIX, -DBL_MANT_DIG/2 ) && f >= f0 )
            {
