...
class QwtDiscreteContinousResponseCurvePaintHelper: public QObject
{
public:
    QwtDiscreteContinousResponseCurvePaintHelper(const QwtDiscreteContinousResponseCurve *curve, int from, int to):
        _curve(curve),
        _from(from),
        _to(to)
    {
...
private:
    const QwtDiscreteContinousResponseCurve *_curve;
    int _from;
    int _to;
};
...
class QwtDiscreteContinousResponseCurve::PrivateData
{
public:
    ...
    PrivateData():
        curveType(Yfx),
        style(QwtDiscreteContinousResponseCurve::Lines),
        reference(0.0),
        attributes(0),
        paintAttributes(0)
    {
    ...
    QwtDiscreteContinousResponseCurve::CurveType curveType;
    QwtDiscreteContinousResponseCurve::CurveStyle style;
    double reference;
...
void QwtDiscreteContinousResponseCurve::run (void)
{
    QPolygonF Polygon;
    ...
    for(int i = Grade ; i < EvaluationInfo.Dots+Grade ; i ++ )
    {
        ...
        try
        {
            ...
            Y->At(i+1) = DiscreteEvaluator->Eval();
            DiscreteOutput.append(Y->At(i).GetFloat());

            X->At(i+1) = mup::Value(((double)1.0)-Kernel->convolvedDot(i-Grade, &DiscreteOutput));

            Polygon.append(QPointF((double)(i-Grade)*EvaluationInfo.Resolution, (double)-X->At(i+1).GetFloat()+1));
...
void QwtDiscreteContinousResponseCurve::valueChangeSlot(QPair <QString, double> VarPair, bool Restart)
{
    ...
    for (var_maptype::iterator item = vmap.begin(); item!=vmap.end(); ++item)
    {
        ...
        if(item->first == VarPair.first.toStdString())
        {
    ...
    QStringList reVars = pSysEvaluator->getExpressionVars();
    for(int i = 0 ; i < reVars.count() ;  i ++ )
    {
        if(reVars.at(i) == VarPair.first)
        {
            pSysEvaluator->setVar(VarPair.first, VarPair.second);
            RegenKernel = true;
            Changed = true;
        }
    }
...
*/
QwtDiscreteContinousResponseCurve::QwtDiscreteContinousResponseCurve(const QwtText &title, QStringList Expression, EvalInfo EvInfo):
    QwtPlotItem(title)
{
    init(Expression, EvInfo);
}
...
/*!
  ...
  \param title Title of the curve
*/
QwtDiscreteContinousResponseCurve::QwtDiscreteContinousResponseCurve(const QString &title, QStringList Expression, EvalInfo EvInfo):
...
//! Destructor
QwtDiscreteContinousResponseCurve::~QwtDiscreteContinousResponseCurve()
{
    this->terminate();
    delete d_xy;
    delete d_data;
...
*/
void QwtDiscreteContinousResponseCurve::init(QStringList Expression, EvalInfo EvInfo)
{
    ...
    X = new Value(EvInfo.Dots*2+Grade,0);
    Y = new Value(EvInfo.Dots*2+Grade,0);
    DiscreteEvaluator->DefineVar("x", Variable(X));
    DiscreteEvaluator->DefineVar("y", Variable(Y));

    int index = Expression.at(0).indexOf(":");
    if(index != -1)
    {
        ExpressionName = Expression.at(0).left(index);
    ...
    //Continous System
    pSysExpression = new ControlExpression(Expression.at(1), "s");
    ...
    for(int i = 0 ; i < EvInfo.Dots+Grade ; i ++ )
    {
        Y->At(i) = 0;
        X->At(i) = 1;
    }
...
*/
void QwtDiscreteContinousResponseCurve::setPaintAttribute(PaintAttribute attribute, bool on)
{
    if ( on )
...
*/
bool QwtDiscreteContinousResponseCurve::testPaintAttribute(PaintAttribute attribute) const
{
...
/*!
  ...
  \sa CurveStyle, style()
*/
void QwtDiscreteContinousResponseCurve::setStyle(CurveStyle style)
{
...
*/
QwtDiscreteContinousResponseCurve::CurveStyle QwtDiscreteContinousResponseCurve::style() const
{
    return d_data->style;
}
...
/*!
  ...
  \sa symbol()
*/
void QwtDiscreteContinousResponseCurve::setSymbol(const QwtSymbol &symbol )
{
...
/*!
    ...
    \sa setSymbol()
*/
const QwtSymbol &QwtDiscreteContinousResponseCurve::symbol() const
{
...
*/
void QwtDiscreteContinousResponseCurve::setPen(const QPen &pen)
{
    if ( pen != d_data->pen )
    {
        d_data->pen = pen;
...
/*!
    ...
    \sa setPen(), brush()
*/
const QPen& QwtDiscreteContinousResponseCurve::pen() const
{
    return d_data->pen;
}
...
/*!
  \brief Assign a brush.
   ...
   and style() != QwtDiscreteContinousResponseCurve::Sticks
...
*/
void QwtDiscreteContinousResponseCurve::setBrush(const QBrush &brush)
{
...
*/
const QBrush& QwtDiscreteContinousResponseCurve::brush() const
{
    return d_data->brush;
}
...
*/
void QwtDiscreteContinousResponseCurve::setData(const double *xData, const double *yData, int size)
{
    delete d_xy;
...
/*!
  ...
  \note Internally the data is stored in a QwtArrayData object
*/
void QwtDiscreteContinousResponseCurve::setData(const QwtArray<double> &xData,
...
*/
#if QT_VERSION < 0x040000
void QwtDiscreteContinousResponseCurve::setData(const QwtArray<QwtDoublePoint> &data)
#else
void QwtDiscreteContinousResponseCurve::setData(const QPolygonF &data)
#endif
{
    delete d_xy;
...
/*!
  ...
  \sa QwtData::copy()
*/
void QwtDiscreteContinousResponseCurve::setData(const QwtData &data)
{
    delete d_xy;
    d_xy = data.copy();
    itemChanged();
...
/*!
  ...
  by QwtDiscreteContinousResponseCurve.
...
*/
void QwtDiscreteContinousResponseCurve::setRawData(const double *xData, const double *yData, int size)
{
    delete d_xy;
...
QwtDoubleRect QwtDiscreteContinousResponseCurve::boundingRect() const
{
    if ( d_xy == NULL )
...
*/
void QwtDiscreteContinousResponseCurve::draw(QPainter *painter,
...
/*!
  ...
  \sa drawCurve(), drawSymbols()
*/
void QwtDiscreteContinousResponseCurve::draw(int from, int to) const
{
    if ( !plot() )
        return;
        ...
        !canvas->testAttribute(Qt::WA_PaintOutsidePaintEvent) )
    {
        /*
          ...
          the QwtDiscreteContinousResponseCurve::draw, but now from inside the paint
          event.
         */

        QwtDiscreteContinousResponseCurvePaintHelper helper(this, from, to);
...
/*!
  ...
  \param painter Painter
  \param style curve style, see QwtDiscreteContinousResponseCurve::CurveStyle
  \param xMap x map
  \param yMap y map
  \param from index of the first point to be painted
...
void QwtDiscreteContinousResponseCurve::drawCurve(QPainter *painter, int style,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
    switch (style)
...
*/
void QwtDiscreteContinousResponseCurve::drawLines(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
    int size = to - from + 1;
...
*/
void QwtDiscreteContinousResponseCurve::drawSticks(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtDiscreteContinousResponseCurve::drawDots(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtDiscreteContinousResponseCurve::drawSteps(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtDiscreteContinousResponseCurve::setCurveAttribute(CurveAttribute attribute, bool on)
{
...
/*!
    ...
    \sa CurveAttribute, setCurveAttribute()
*/
bool QwtDiscreteContinousResponseCurve::testCurveAttribute(CurveAttribute attribute) const
{
...
*/
void QwtDiscreteContinousResponseCurve::setCurveType(CurveType curveType)
{
...
*/
QwtDiscreteContinousResponseCurve::CurveType QwtDiscreteContinousResponseCurve::curveType() const
{
...
*/
void QwtDiscreteContinousResponseCurve::setCurveFitter(QwtCurveFitter *curveFitter)
{
...
/*!
  ...
  \return Curve fitter
*/
QwtCurveFitter *QwtDiscreteContinousResponseCurve::curveFitter() const
{
...
*/
void QwtDiscreteContinousResponseCurve::fillCurve(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    QwtPolygon &pa) const
{
    if ( d_data->brush.style() == Qt::NoBrush )
...
*/
void QwtDiscreteContinousResponseCurve::closePolyline(
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    QwtPolygon &pa) const
{
    ...
    if ( d_data->curveType == QwtDiscreteContinousResponseCurve::Xfy )
    {
        pa.setPoint(sz,
...
*/
void QwtDiscreteContinousResponseCurve::drawSymbols(QPainter *painter, const QwtSymbol &symbol,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
/*!
  ...
  of the baseline depends on the CurveType. With QwtDiscreteContinousResponseCurve::Yfx,
  the baseline is interpreted as a horizontal line at y = baseline(),
  with QwtDiscreteContinousResponseCurve::Yfy, it is interpreted as a vertical line at
  x = baseline().
  \param reference baseline
  \sa baseline(), setBrush(), setStyle(), setCurveType()
*/
void QwtDiscreteContinousResponseCurve::setBaseline(double reference)
{
...
/*!
    ...
    \sa setBaseline()
*/
double QwtDiscreteContinousResponseCurve::baseline() const
{
...
/*!
  ...
  \sa setData()
*/
int QwtDiscreteContinousResponseCurve::dataSize() const
{
    return d_xy->size();
}
...
/*!
  ...
  \note closestPoint() implements a dumb algorithm, that iterates
        over all points
*/
int QwtDiscreteContinousResponseCurve::closestPoint(const QPoint &pos, double *dist) const
{
...
void QwtDiscreteContinousResponseCurve::updateLegend(QwtLegend *legend) const
{
    if ( !legend )
        return;
    ...
    else if (policy == QwtLegend::AutoIdentifier)
    {
        ...
        if (QwtDiscreteContinousResponseCurve::NoCurve != style())
        {
            legendItem->setCurvePen(pen());
