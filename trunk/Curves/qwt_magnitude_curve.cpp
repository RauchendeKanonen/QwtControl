...
class QwtMagnitudeCurvePaintHelper: public QObject
{
public:
    QwtMagnitudeCurvePaintHelper(const QwtMagnitudeCurve *curve, int from, int to):
        _curve(curve),
        _from(from),
        _to(to)
    {
...
private:
    const QwtMagnitudeCurve *_curve;
    int _from;
    int _to;
};
...
class QwtMagnitudeCurve::PrivateData
{
public:
    ...
    QwtMagnitudeCurve::CurveType curveType;
    QwtMagnitudeCurve::CurveStyle style;
    double reference;
...
void QwtMagnitudeCurve::run (void)
{
    QPolygonF Polygon;
    ...
    AmplitudeMarker->setXValue(-1.0);
    AmplitudeMarker->setYValue(-1.0);
    ...
    for(int i = 0 ; i < dots ; i ++ )
    {
        ...
        if(i > 0)
        {
            if((a_1 > 1.0) && (a < 1.0))
            {
                AmplitudeMarker->setXValue(w-EvaluationInfo.Resolution/2);
                AmplitudeMarker->setYValue(a);
                emit amplitudeMarkerChangeSignal(w);
            }
        }
        a_1 = a;
        Polygon.append(QPointF(w, a));
    }

    emit dataReadySig(Polygon);
}

void QwtMagnitudeCurve::dataReadySlot(QPolygonF Polygon)
{
    setData(Polygon);
    if(plot())
    {
        AmplitudeMarker->attach(plot());
        plot()->replot();
    }
}

void QwtMagnitudeCurve::markerChangeSlot(QPair<QString,double> MarkerPair)
{
    QwtPlot *Plot = plot();
    if(Plot)
    {
...
//! Constructor
QwtMagnitudeCurve::QwtMagnitudeCurve(ControlExpression *Expression, EvalInfo EvInfo):
    QwtPlotItem(QwtText())
{
...
*/
QwtMagnitudeCurve::QwtMagnitudeCurve(const QwtText &title, ControlExpression *Expression, EvalInfo EvInfo):
    QwtPlotItem(title)
{
    init(Expression, EvInfo);
}
...
/*!
  ...
  \param title Title of the curve
*/
QwtMagnitudeCurve::QwtMagnitudeCurve(const QString &title, ControlExpression *Expression, EvalInfo EvInfo):
...
//! Destructor
QwtMagnitudeCurve::~QwtMagnitudeCurve()
{
    this->terminate();
    delete d_xy;
    delete d_data;
...
*/
void QwtMagnitudeCurve::init(ControlExpression *Expression, EvalInfo EvInfo)
{
...
int QwtMagnitudeCurve::rtti() const
{
    return QwtPlotItem::Rtti_PlotUserItem+Rtti_PlotMagnitude;
}
...
*/
void QwtMagnitudeCurve::setPaintAttribute(PaintAttribute attribute, bool on)
{
    if ( on )
...
*/
bool QwtMagnitudeCurve::testPaintAttribute(PaintAttribute attribute) const
{
...
/*!
  ...
  \sa CurveStyle, style()
*/
void QwtMagnitudeCurve::setStyle(CurveStyle style)
{
...
*/
QwtMagnitudeCurve::CurveStyle QwtMagnitudeCurve::style() const
{
    return d_data->style;
}
...
/*!
  ...
  \sa symbol()
*/
void QwtMagnitudeCurve::setSymbol(const QwtSymbol &symbol )
{
...
/*!
    ...
    \sa setSymbol()
*/
const QwtSymbol &QwtMagnitudeCurve::symbol() const
{
...
*/
void QwtMagnitudeCurve::setPen(const QPen &pen)
{
    if ( pen != d_data->pen )
    {
        d_data->pen = pen;
...
/*!
    ...
    \sa setPen(), brush()
*/
const QPen& QwtMagnitudeCurve::pen() const
{
    return d_data->pen;
}
...
/*!
  \brief Assign a brush.
   ...
   and style() != QwtMagnitudeCurve::Sticks
...
*/
void QwtMagnitudeCurve::setBrush(const QBrush &brush)
{
...
*/
const QBrush& QwtMagnitudeCurve::brush() const
{
    return d_data->brush;
}
...
*/
void QwtMagnitudeCurve::setData(const double *xData, const double *yData, int size)
{
    delete d_xy;
...
/*!
  ...
  \note Internally the data is stored in a QwtArrayData object
*/
void QwtMagnitudeCurve::setData(const QwtArray<double> &xData,
...
*/
#if QT_VERSION < 0x040000
void QwtMagnitudeCurve::setData(const QwtArray<QwtDoublePoint> &data)
#else
void QwtMagnitudeCurve::setData(const QPolygonF &data)
#endif
{
    delete d_xy;
...
/*!
  ...
  \sa QwtData::copy()
*/
void QwtMagnitudeCurve::setData(const QwtData &data)
{
    delete d_xy;
    d_xy = data.copy();
    itemChanged();
...
*/
void QwtMagnitudeCurve::setRawData(const double *xData, const double *yData, int size)
{
    delete d_xy;
...
QwtDoubleRect QwtMagnitudeCurve::boundingRect() const
{
    if ( d_xy == NULL )
...
*/
void QwtMagnitudeCurve::draw(QPainter *painter,
...
/*!
  ...
  \sa drawCurve(), drawSymbols()
*/
void QwtMagnitudeCurve::draw(int from, int to) const
{
    if ( !plot() )
        return;
        ...
        !canvas->testAttribute(Qt::WA_PaintOutsidePaintEvent) )
    {
        /*
          ...
          the QwtMagnitudeCurve::draw, but now from inside the paint
          event.
         */

        QwtMagnitudeCurvePaintHelper helper(this, from, to);
...
/*!
  ...
  \param painter Painter
  \param style curve style, see QwtMagnitudeCurve::CurveStyle
  \param xMap x map
  \param yMap y map
  \param from index of the first point to be painted
...
void QwtMagnitudeCurve::drawCurve(QPainter *painter, int style,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
    switch (style)
...
*/
void QwtMagnitudeCurve::drawLines(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
    int size = to - from + 1;
...
*/
void QwtMagnitudeCurve::drawSticks(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtMagnitudeCurve::drawDots(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtMagnitudeCurve::drawSteps(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtMagnitudeCurve::setCurveAttribute(CurveAttribute attribute, bool on)
{
...
/*!
    ...
    \sa CurveAttribute, setCurveAttribute()
*/
bool QwtMagnitudeCurve::testCurveAttribute(CurveAttribute attribute) const
{
...
*/
void QwtMagnitudeCurve::setCurveType(CurveType curveType)
{
...
*/
QwtMagnitudeCurve::CurveType QwtMagnitudeCurve::curveType() const
{
...
*/
void QwtMagnitudeCurve::setCurveFitter(QwtCurveFitter *curveFitter)
{
...
/*!
  ...
  \return Curve fitter
*/
QwtCurveFitter *QwtMagnitudeCurve::curveFitter() const
{
...
*/
void QwtMagnitudeCurve::fillCurve(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    QwtPolygon &pa) const
{
    if ( d_data->brush.style() == Qt::NoBrush )
...
*/
void QwtMagnitudeCurve::closePolyline(
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    QwtPolygon &pa) const
{
    ...
    if ( d_data->curveType == QwtMagnitudeCurve::Xfy )
    {
        pa.setPoint(sz,
...
*/
void QwtMagnitudeCurve::drawSymbols(QPainter *painter, const QwtSymbol &symbol,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
/*!
  ...
  of the baseline depends on the CurveType. With QwtMagnitudeCurve::Yfx,
  the baseline is interpreted as a horizontal line at y = baseline(),
  with QwtMagnitudeCurve::Yfy, it is interpreted as a vertical line at
  x = baseline().
  \param reference baseline
  \sa baseline(), setBrush(), setStyle(), setCurveType()
*/
void QwtMagnitudeCurve::setBaseline(double reference)
{
...
/*!
    ...
    \sa setBaseline()
*/
double QwtMagnitudeCurve::baseline() const
{
...
/*!
  ...
  \sa setData()
*/
int QwtMagnitudeCurve::dataSize() const
{
    return d_xy->size();
}
...
/*!
  ...
  \note closestPoint() implements a dumb algorithm, that iterates
        over all points
*/
int QwtMagnitudeCurve::closestPoint(const QPoint &pos, double *dist) const
{
...
void QwtMagnitudeCurve::updateLegend(QwtLegend *legend) const
{
    if ( !legend )
        return;
    ...
    else if (policy == QwtLegend::AutoIdentifier)
    {
        ...
        if (QwtMagnitudeCurve::NoCurve != style())
        {
            legendItem->setCurvePen(pen());
