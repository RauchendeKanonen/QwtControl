...
class QwtRootLocusCurvePaintHelper: public QObject
{
public:
    QwtRootLocusCurvePaintHelper(const QwtRootLocusCurve *curve, int from, int to):
        _curve(curve),
        _from(from),
        _to(to)
    {
...
private:
    const QwtRootLocusCurve *_curve;
    int _from;
    int _to;
};
...
class QwtRootLocusCurve::PrivateData
{
public:
    ...
    QwtRootLocusCurve::CurveType curveType;
    QwtRootLocusCurve::CurveStyle style;
    double reference;
...
void QwtRootLocusCurve::run (void)
{
    QPolygonF Polygon;
...
void QwtRootLocusCurve::dataReadySlot(QPolygonF Polygon)
{
    setData(Polygon);
...
void QwtRootLocusCurve::markerChangeSlot(QPair<QString,double> MarkerPair)
{
    ...
    if(Plot && MarkerPair.first == this->pImagEval->indepVarName())
    {
        ...
        im = pImagEval->eval(InVal);
        re = pRealEval->eval(InVal);
        PoleLocation->setYValue(im);
        PoleLocation->setXValue(re);

        InVal = 10e9;
        im = pImagEval->eval(InVal);
        re = pRealEval->eval(InVal);
        RootLocation->setYValue(im);
        RootLocation->setXValue(re);
...
//! Constructor
QwtRootLocusCurve::QwtRootLocusCurve(ControlExpression *Expression, EvalInfo EvInfo):
    QwtPlotItem(QwtText())
{
...
*/
QwtRootLocusCurve::QwtRootLocusCurve(const QwtText &title, ControlExpression *Expression, EvalInfo EvInfo):
    QwtPlotItem(title)
{
    init(Expression, EvInfo);
}
...
/*!
  ...
  \param title Title of the curve
*/
QwtRootLocusCurve::QwtRootLocusCurve(const QString &title, ControlExpression *Expression, EvalInfo EvInfo):
...
//! Destructor
QwtRootLocusCurve::~QwtRootLocusCurve()
{
    this->terminate();
    delete d_xy;
    delete d_data;
...
*/
void QwtRootLocusCurve::init(ControlExpression *Expression, EvalInfo EvInfo)
{
    ...
    PoleLocation->setSymbol( QwtSymbol( QwtSymbol::XCross,
                                                     QColor(Color), QColor(Color), QSize( 15, 15 ) ) );
    RootLocation->setSymbol( QwtSymbol( QwtSymbol::Ellipse,
                                                     QColor(Color), QColor(Color), QSize( 10, 10 ) ) );

    Marker = new QwtPlotMarker();
    Marker->setSymbol( QwtSymbol( QwtSymbol::Cross,
...
int QwtRootLocusCurve::rtti() const
{
    return QwtPlotItem::Rtti_PlotUserItem+Rtti_PlotRootLocus;
}
...
*/
void QwtRootLocusCurve::setPaintAttribute(PaintAttribute attribute, bool on)
{
    if ( on )
...
*/
bool QwtRootLocusCurve::testPaintAttribute(PaintAttribute attribute) const
{
...
/*!
  ...
  \sa CurveStyle, style()
*/
void QwtRootLocusCurve::setStyle(CurveStyle style)
{
...
*/
QwtRootLocusCurve::CurveStyle QwtRootLocusCurve::style() const
{
    return d_data->style;
}
...
/*!
  ...
  \sa symbol()
*/
void QwtRootLocusCurve::setSymbol(const QwtSymbol &symbol )
{
...
/*!
    ...
    \sa setSymbol()
*/
const QwtSymbol &QwtRootLocusCurve::symbol() const
{
...
*/
void QwtRootLocusCurve::setPen(const QPen &pen)
{
    if ( pen != d_data->pen )
    {
        d_data->pen = pen;
...
/*!
    ...
    \sa setPen(), brush()
*/
const QPen& QwtRootLocusCurve::pen() const
{
    return d_data->pen;
}
...
/*!
  \brief Assign a brush.
   ...
   and style() != QwtRootLocusCurve::Sticks
...
*/
void QwtRootLocusCurve::setBrush(const QBrush &brush)
{
...
*/
const QBrush& QwtRootLocusCurve::brush() const
{
    return d_data->brush;
}
...
*/
void QwtRootLocusCurve::setData(const double *xData, const double *yData, int size)
{
    delete d_xy;
...
/*!
  ...
  \note Internally the data is stored in a QwtArrayData object
*/
void QwtRootLocusCurve::setData(const QwtArray<double> &xData,
...
*/
#if QT_VERSION < 0x040000
void QwtRootLocusCurve::setData(const QwtArray<QwtDoublePoint> &data)
#else
void QwtRootLocusCurve::setData(const QPolygonF &data)
#endif
{
    delete d_xy;
...
/*!
  ...
  \sa QwtData::copy()
*/
void QwtRootLocusCurve::setData(const QwtData &data)
{
    delete d_xy;
    d_xy = data.copy();
    itemChanged();
...
*/
void QwtRootLocusCurve::setRawData(const double *xData, const double *yData, int size)
{
    delete d_xy;
...
QwtDoubleRect QwtRootLocusCurve::boundingRect() const
{
    if ( d_xy == NULL )
...
*/
void QwtRootLocusCurve::draw(QPainter *painter,
...
/*!
  ...
  \sa drawCurve(), drawSymbols()
*/
void QwtRootLocusCurve::draw(int from, int to) const
{
    if ( !plot() )
        return;
        ...
        !canvas->testAttribute(Qt::WA_PaintOutsidePaintEvent) )
    {
        /*
          ...
          the QwtRootLocusCurve::draw, but now from inside the paint
          event.
         */

        QwtRootLocusCurvePaintHelper helper(this, from, to);
...
/*!
  ...
  \param painter Painter
  \param style curve style, see QwtRootLocusCurve::CurveStyle
  \param xMap x map
  \param yMap y map
  \param from index of the first point to be painted
...
void QwtRootLocusCurve::drawCurve(QPainter *painter, int style,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
    switch (style)
...
*/
void QwtRootLocusCurve::drawLines(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
    int size = to - from + 1;
...
*/
void QwtRootLocusCurve::drawSticks(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtRootLocusCurve::drawDots(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtRootLocusCurve::drawSteps(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
*/
void QwtRootLocusCurve::setCurveAttribute(CurveAttribute attribute, bool on)
{
...
/*!
    ...
    \sa CurveAttribute, setCurveAttribute()
*/
bool QwtRootLocusCurve::testCurveAttribute(CurveAttribute attribute) const
{
...
*/
void QwtRootLocusCurve::setCurveType(CurveType curveType)
{
...
*/
QwtRootLocusCurve::CurveType QwtRootLocusCurve::curveType() const
{
...
*/
void QwtRootLocusCurve::setCurveFitter(QwtCurveFitter *curveFitter)
{
...
/*!
  ...
  \return Curve fitter
*/
QwtCurveFitter *QwtRootLocusCurve::curveFitter() const
{
...
*/
void QwtRootLocusCurve::fillCurve(QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    QwtPolygon &pa) const
{
    if ( d_data->brush.style() == Qt::NoBrush )
...
*/
void QwtRootLocusCurve::closePolyline(
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    QwtPolygon &pa) const
{
    ...
    if ( d_data->curveType == QwtRootLocusCurve::Xfy )
    {
        pa.setPoint(sz,
...
*/
void QwtRootLocusCurve::drawSymbols(QPainter *painter, const QwtSymbol &symbol,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    int from, int to) const
{
...
/*!
  ...
  of the baseline depends on the CurveType. With QwtRootLocusCurve::Yfx,
  the baseline is interpreted as a horizontal line at y = baseline(),
  with QwtRootLocusCurve::Yfy, it is interpreted as a vertical line at
  x = baseline().
  \param reference baseline
  \sa baseline(), setBrush(), setStyle(), setCurveType()
*/
void QwtRootLocusCurve::setBaseline(double reference)
{
...
/*!
    ...
    \sa setBaseline()
*/
double QwtRootLocusCurve::baseline() const
{
...
/*!
  ...
  \sa setData()
*/
int QwtRootLocusCurve::dataSize() const
{
    return d_xy->size();
}
...
/*!
  ...
  \note closestPoint() implements a dumb algorithm, that iterates
        over all points
*/
int QwtRootLocusCurve::closestPoint(const QPoint &pos, double *dist) const
{
...
void QwtRootLocusCurve::updateLegend(QwtLegend *legend) const
{
    if ( !legend )
        return;
    ...
    else if (policy == QwtLegend::AutoIdentifier)
    {
        ...
        if (QwtRootLocusCurve::NoCurve != style())
        {
            legendItem->setCurvePen(pen());
